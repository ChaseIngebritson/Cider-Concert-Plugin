...
Vue.component('plugin.concerts', {
  ...
  methods: {
    ...
    async setActiveConcert(concert) {
      ...
      if (!attractions?.length || isFestival // Festivals usually have their name as the first artist, so just skip searching for artists in this
      ) return this.activeArtist = null;
      const artists = await CiderConcertsPlugin.searchArtists(attractions[0].name);
      if (!artists?.length) this.activeArtist = null;else this.activeArtist = artists[0];
    },

    onPageClick(direction) {
      if (direction === 'prev') this.page--;else this.page++;
    }
  ...
  watch: {
    artist() {
      if (!this.artist) CiderConcertsPlugin.removeLocalStorage('artist');else CiderConcertsPlugin.updateLocalStorage('artist', this.artist);
      this.searchConcert();
    },
